...
from opengnn.encoders import GGNNEncoder
...
from opengnn.models.graph_key_to_sequence import GraphKeyToSequence
from opengnn.models.graph_to_classify import GraphToClassify
from opengnn.utils import read_jsonl_gz_file
from opengnn.utils.attention_wrapper import BahdanauAttention
...
def main():
    ...
    parser.add_argument('--validation_metric', default='rouge', type=str,
                        help="The metric to compare models_lib between validations")
    ...
    parser.add_argument('--attend_all_nodes', default=True, action='store_true',
    ...
    parser.add_argument('--copy_attention', default=True, action='store_true',
                        help="Number of epochs to train the model")
    ...
    parser.add_argument('--classify', default=False, action='store_true',
                        help="If enabled, run the classify model")

    # arguments for loading data
    parser.add_argument('--train_source_file', type=str,
                        help="Path to the jsonl.gz file containing the train input graphs")
    parser.add_argument('--train_target_file', type=str,
                        help="Path to the jsonl.gz file containing the train input graphs")
    parser.add_argument('--valid_source_file', type=str,
                        help="Path to the jsonl.gz file containing the valid input graphs")
    parser.add_argument('--valid_target_file', type=str,
                        help="Path to the jsonl.gz file containing the valid input graphs")
    parser.add_argument('--infer_source_file', default=None,
                        help="Path to the jsonl.gz file in which we wish to do inference "
                             "after training is complete")
    parser.add_argument('--infer_target_file', type=str,
                        help="Path to the jsonl.gz file containing the infer summaries")
    parser.add_argument('--infer_predictions_file', default=None,
                        help="Path to the file to save the results from inference")
    parser.add_argument('--node_vocab_file', type=str,
                        help="Path to the json containing the dataset")
    parser.add_argument('--edge_vocab_file', type=str,
                        help="Path to the json containing the dataset")
...
# 训练第三条python，也就是训练方法名生成模型
        ...
        for train_step in range(initial_step + 1, args.train_steps + 1):
            ...
            if train_step % args.checkpoint_interval == 0:
                print("saving current model.. {}".format(args.model_name))
            ...
            if train_step % args.validation_interval == 0:
                valid_loss, valid_rouge, rouge_2, rouge_lcs = evaluate(
                    session,
                    model,
                    valid_iterator,
                    valid_tb_loss,
                    predictions,
                    valid_targets)
                print("eval loss: %0.4f, eval rouge: %0.4f, eval rouge_2: %0.4f, eval rouge_lcs: %0.4f" % (
                    valid_loss, valid_rouge, rouge_2, rouge_lcs))
                ...
                if args.validation_metric == "rouge":
                    ...
                    if valid_rouge > best_metric:
                        ...
                        print('worse epoch:{} best metric:{}'.format(worse_epochs, best_metric))
                        worse_epochs += 1
...
def evaluate(session,
    ...
    rouge, rouge_2, rouge_lcs = compute_rouge(valid_predictions, targets)
    return loss, rouge, rouge_2, rouge_lcs
...
def build_model(args):
    ...
    if args.copy_attention and not args.classify:
...
def infer(model, args):
    ...
    targets = read_jsonl_gz_file(args.infer_target_file)
    ...
    with tf.Session(config=session_config) as session:
        ...
        print("loading model")
        saver.restore(session, os.path.join(args.checkpoint_dir, "current.ckpt-120000"))
    ...
    rouge = compute_rouge(infer_predictions, targets)
    print('rouge:{}'.format(rouge))
    pred_dict = dict(zip(range(len(infer_predictions)), infer_predictions))
...
def compute_rouge(predictions, targets):
    predictions = [" ".join(prediction).lower().strip().replace('.', 'EMPTY') for prediction in predictions]
    predictions = [prediction if prediction else "EMPTY" for prediction in predictions]
    targets = [" ".join(target).lower().strip().replace('.', 'EMPTY') for target in targets]
    ...
    try:
        ...
        print('value error')
        return 0.0
    print(str(scores['rouge-1']['f'])+str( scores['rouge-2']['f'])+str( scores['rouge-l']['f']))
    print(str(scores['rouge-1']['r'])+str( scores['rouge-2']['r'])+str( scores['rouge-l']['r']))
    print(str(scores['rouge-1']['p'])+str( scores['rouge-2']['p'])+str( scores['rouge-l']['p']))
