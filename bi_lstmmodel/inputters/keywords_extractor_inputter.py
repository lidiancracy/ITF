...
class KeywordsExtractorInputter(Inputter):
    def __init__(self,
    ...
    def extract_tensors(self):
        ...
        def _tensor_extractor(sample):
            ...
            mul_task_id = sample['mul_task_id']
            # str_tokens = [sample['node_labels'][i] for i in sample['backbone_sequence']]
            str_tokens = sample['node_labels']
            return {
                **tensor,
                "primary_path": backbone_sequence,
                "mul_task_id": mul_task_id,
        ...
        tensor_types.update({
            ...
            "mul_task_id": tf.int64,
            "str_tokens": tf.string,
        })
        tensor_shapes.update({
            "primary_path": tf.TensorShape([None]),
            "mul_task_id": tf.TensorShape([None]),
            "str_tokens": tf.TensorShape([None]),
        })
    ...
    def _process(self, data, input_data):
        ...
        data['mul_task_id_length'] = tf.shape(data['mul_task_id'])[0]
        data['str_tokens_length'] = tf.shape(data['str_tokens'])[0]
    ...
    def batch(self, dataset, batch_size):
        ...
        batch_fn_map = {("primary_path", "primary_path_length"): _batch,
                        ("mul_task_id", "mul_task_id_length"): _batch,
                        ("str_tokens", "str_tokens_length"): _batch, }
