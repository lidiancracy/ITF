...
def get_key_words_tokens(tokens, key_words):
    key_words_tokens = []
    for word in key_words:
        key_words_tokens.append(tokens[word])
    return np.array(key_words_tokens, dtype=object)
...
class CopyingTokenEmbedder(TokenEmbedder):
    ...
    def _process(self, data, input_data):
        if input_data is None:
            ...
            data['key_words_tokens'] = tf.py_func(
                lambda labels, key_words:
                    get_key_words_tokens(
                        labels, key_words),
                inp=(data['labels'], data['key_words']),
                Tout=tf.string)
            data['key_words_tokens'].set_shape([None])

            # perform the extend lookup on key tokens by ge
            key_ids = self.output_vocabulary.lookup(data['key_words_tokens'])
            data['key_out_ids'] = tf.py_func(
                lambda key_words_tokens, key_ids, labels, out_ids:
                extended_lookup(
                    key_words_tokens, key_ids, labels, out_ids,
                    self.output_vocabulary_size, self.output_vocabulary_size - 1)[0],
                inp=(data['key_words_tokens'], key_ids, data['labels'], out_ids),
                Tout=tf.int64)
            data['key_out_ids'].set_shape([None])
