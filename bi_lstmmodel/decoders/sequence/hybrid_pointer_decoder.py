...
class HybridPointerDecoder(RNNDecoder):
    ...
    def decode(self,
        ...
        cell, initial_state = build_cell(
            ...
            extend_memory=extend_memory,
            extend_memory_sequence=extend_memory_sequence,
            extend_memory_length=extend_memory_length,
            mode=mode,
        ...
        copying_mechanism = unnormalized_luong_attention(
            ...
            memory_sequence_len,
            extend_memory,
            extend_memory_sequence,
            extend_memory_length)
    ...
    def dynamic_decode_and_search(self,
        ...
        if extend_memory is not None:
            extend_memory = tf.contrib.seq2seq.tile_batch(
                extend_memory, multiplier=beam_width)
        if extend_memory_sequence is not None:
            extend_memory_sequence = tf.contrib.seq2seq.tile_batch(
                extend_memory_sequence, multiplier=beam_width)
        if extend_memory_length is not None:
            extend_memory_length = tf.contrib.seq2seq.tile_batch(
                extend_memory_length, multiplier=beam_width)
        if extend_memory_out_ids is not None:
            extend_memory_out_ids = tf.contrib.seq2seq.tile_batch(
                extend_memory_out_ids, multiplier=beam_width)
