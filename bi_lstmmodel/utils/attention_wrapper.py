...
class LuongAttention(_BaseAttentionMechanism):
    ...
    def __init__(self,
                 ...
                 memory_sequence_length=None,
                 extend_memory=None,
                 extend_memory_sequence=None,
                 extend_memory_length=None,
                 scale=False,
                 probability_fn=None,
                 score_mask_value=None,
                 dtype=None,
        ...
        self._scale = scale
        self._name = name
        self._extend_memory = extend_memory
        self._extend_memory_length = extend_memory_length
        self._extend_memory_sequence = extend_memory_sequence
...
def _compute_attention(attention_mechanism, cell_output, attention_state,
    ...
    return attention, alignments, next_attention_state, context_
...
class AttentionWrapper(rnn_cell_impl.RNNCell):
    ...
    def __init__(self,
                 cell,
                 attention_mechanism,
                 extend_attention_mechanism=None,
        ...
        **NOTE** If you are using the `BeamSearchDecoder` with a cell wrapped in
        ...
        if attention_layer_size is not None:
            ...
            self._extend_attention_layers = tuple(
                layers_core.Dense(
                    attention_layer_size,
                    name="extend_attention_layer",
                    use_bias=False,
                    dtype=attention_mechanisms[i].dtype)
                for i, attention_layer_size in enumerate(attention_layer_sizes))
        ...
        self._extend_attention_mechanism = extend_attention_mechanism
        self.concat_attention_linear = tf.layers.Dense(256)
    ...
    def call(self, inputs, state):
        ...
        for i, attention_mechanism in enumerate(self._attention_mechanisms):
            attention, alignments, next_attention_state, context_ = self._attention_fn(
        ...
        alignments = array_ops.concat(all_alignments, 1)

        key_attention, key_alignments, key_next_attention_state, \
        key_context_ = self._attention_fn(
            self._extend_attention_mechanism, cell_output, previous_attention_state[0],
            self._extend_attention_layers[0] if self._extend_attention_layers else None)

        concat_context = self.concat_attention_linear(tf.concat([context_, key_context_], axis=-1))

        next_state = AttentionWrapperState(
            time=state.time + 1,
            cell_state=next_cell_state,
            attention=concat_context,
            attention_state=self._item_or_tuple(all_attention_states),
            alignments=self._item_or_tuple(all_alignments),
            alignment_history=self._item_or_tuple(maybe_all_histories))

        if self._output_attention:
            return attention, next_state, concat_context, alignments
        else:
            return cell_output, next_state, concat_context, alignments
