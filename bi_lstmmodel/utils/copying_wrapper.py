...
# from tensorflow.contrib.seq2seq.python.ops.attention_wrapper import LuongAttention, AttentionMechanism
from opengnn.utils.attention_wrapper import *


def unnormalized_luong_attention(*args):
    return LuongAttention(*args, probability_fn=lambda scores: scores)
...
class CopyingWrapper(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self,
        ...
        if not isinstance(copying_mechanism, AttentionMechanism):
            raise TypeError(
        ...
        self.p_gen_linear = tf.layers.Dense(1, use_bias=False)
    ...
    def call(self, inputs, state):
        ...
        cell_output, next_cell_state, context_, _ = \
            self._cell(inputs, cell_state)

        if isinstance(next_cell_state, AttentionWrapperState):
        ...
        p_gen = tf.sigmoid(self.p_gen_linear(tf.concat(
            [inputs, self._get_state(raw_cell_state, lstm_output_only=True), context_], axis=-1)))
        base_logits = p_gen * base_logits
        copying_logits = 0.5 * (1 - p_gen) * copying_logits
