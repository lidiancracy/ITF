...
class GraphKeyToSequence(Model):
    def __init__(self,
    ...
    def __call__(self,
        ...
        key_words = features["key_words"]
        key_words_length = features["key_words_length"]
        key_words_features = batch_gather(node_features, key_words)
        key_out_ids = features['key_out_ids']
        ...
        with tf.variable_scope("keywords_encoder", reuse=tf.AUTO_REUSE):
            fwd_cell = build_cell(
                256 / 2, 1,
            ...
            key_words_representations, rnn_state = tf.nn.bidirectional_dynamic_rnn(
                cell_fw=fwd_cell,
                cell_bw=bwd_cell,
                inputs=key_words_features,
                sequence_length=key_words_length,
                dtype=tf.float32,
                swap_memory=True)

            key_words_representations = tf.concat(key_words_representations, axis=-1)
            ...
            key_words_state = tf.contrib.framework.nest.pack_sequence_as(rnn_state[0], f_rnn_state)
            key_words_state = key_words_state.h

        with tf.variable_scope("co_gate", reuse=tf.AUTO_REUSE):
            node_filter_g = tf.layers.Dense(
                name='node_filter_g',
                units=256,
                use_bias=False
            )

            key_filter_g = tf.layers.Dense(
                name='key_filter_g',
                units=256,
                use_bias=False
            )

            node_filter_k = tf.layers.Dense(
                name='node_filter_k',
                units=256,
                use_bias=False
            )

            key_filter_k = tf.layers.Dense(
                name='key_filter_k',
                units=256,
                use_bias=False
            )

            node_filter_gate = tf.sigmoid(node_filter_g(node_representations)
                                          + tf.expand_dims(key_filter_g(key_words_state), 1))
            node_representations = node_filter_gate * node_representations

            key_filter_gate = tf.sigmoid(key_filter_k(key_words_representations)
                                          + tf.expand_dims(node_filter_k(initial_state),1))
            key_words_representations = key_filter_gate * key_words_representations
        ...
        if labels is not None:
            ...
            with tf.variable_scope("decoder",reuse=tf.AUTO_REUSE) as scope:
                ...
                _, logits, decoder_loss = self.decoder.decode(
                    ...
                    extend_memory=key_words_representations,
                    extend_memory_length=key_words_length,
                    extend_memory_out_ids=key_out_ids,
                    mode=mode)
